...
## Docs Preview Index (for external consumers)

This theme now exposes a generated JSON index that can be consumed by external applications (for example, a Storybook help/preview component):

`/assets/data/docs-preview-index.json`

The JSON is generated from pages using `layout: doc` and includes:
...
The `summary` value is resolved automatically with this priority:

1. `preview_summary`
2. `excerpt`
3. stripped/truncated page content

This means existing repositories do not need immediate content changes to produce a preview summary.

### Optional front matter for better previews

You can improve the generated preview data by adding optional fields in any page:

```yml
---
title: My article
excerpt: Short summary shown in listings
preview_summary: Explicit summary for external preview cards
preview_image: /assets/images/path/to/image.png
preview_tags:
   - setup
   - configuration
doc_key: setup/config
---
```
